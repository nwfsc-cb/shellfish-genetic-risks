...
# years
pre_farm_years = 50 # num of years first (of three) section of the model
farm_years = 50 # num of years second section of the model
post_farm_years = 50 # num of years third section of the model

# wild popdy
num_wild_pops = 3 # number wild pops (model structure currently only handles 3)
wild_N_init = 3000 # initial wild pop size
...
args.batch = '_'.join([str(args.batch),str(args.coreid)])
...
logfilename = '_'.join(['batch',args.batch,'log.txt']) # name log file with batch id
...
bstock_report = open(''.join(['batch_', args.batch, '_bstock_report.txt']), 'w')
bstock_report.write('Rep\tYear\tIndID_survivedAgeX\n')
if make_report_adults:
    track_for_Ne = open(''.join(['batch_', args.batch, '_trackingForNE.txt']), 'w')
...
if make_report_all_inds:
    life_hist_report = open(''.join(['batch_', args.batch, '_life_hist_report.txt']), 'w')
...
for rep in range(args.reps):
    ...
    if args.freq is not None: # if provided input file for starting AFs at equilibrium
        initEqAFs(loci_for_fst= loci_for_fst, eq_afs_path=args.freq)
    ...
    for year in np.arange(pre_farm_years):
        ...
        if year == pre_farm_years-1: # in last year, save population as object to get rvar temporal fst
            pop.save(''.join(['batch_', args.batch, '_t0.pop']))
    ...
    pop_t0 = sim.loadPopulation(''.join(['batch_', args.batch, '_t0.pop']))
...
# for pop_rvar_dict 
pop_rvar_outfile = open(''.join(['batch_', args.batch, '_pop_rvars.txt']), 'w')
        ...
        for sp in pop_rvar_dict[rep][year]:
            for rvar in pop_rvar_dict[rep][year][sp]:
                if sp !='farm':
                    ...
                    sp_name = sp
                pop_rvar_outfile.write('\t'.join([str(args.coreid), str(rep), str(year), sp_name,  
                                                  rvar, str(pop_rvar_dict[rep][year][sp][rvar])]))
                pop_rvar_outfile.write('\n')
pop_rvar_outfile.close()

# for harvest
harvest_outfile = open(''.join(['batch_', args.batch, '_harvest.txt']), 'w')
harvest_outfile.write('Srep\tRep\tYear\tIndsHarvested\n')
for rep in harvest_dict:
    for year in harvest_dict[rep]:
        harvest_outfile.write('\t'.join([str(args.coreid), str(rep), str(year), str(harvest_dict[rep][year])]))
        harvest_outfile.write('\n')
harvest_outfile.close()

if track_AFs == True:
    afs_outfile = open(''.join(['batch_', args.batch, '_AFs.txt']), 'w')
            ...
            for sp in afs_dict[rep][year]:
                ...
                for l_idx in biallelic_loci:
                    if l_idx in loci_for_fst:
                        ...
                        for allele in [0,1]:
                            advantage = "NA"
                            afs_outfile.write('\t'.join([str(args.coreid), str(rep), str(year), sp_name, 
...
pop_pair_rvar_outfile = open(''.join(['batch_', args.batch, '_pop_pair_rvars.txt']), 'w')
    ...
    for year in pop_pair_rvar_dict[rep]:
        for pop_pair in pop_pair_rvar_dict[rep][year]:
            for rvar in pop_pair_rvar_dict[rep][year][pop_pair]:
                pop_pair_rvar_outfile.write('\t'.join([str(args.coreid), str(rep), str(year), pop_pair, rvar, 
...
temp_rvar_outfile = open(''.join(['batch_', args.batch, '_temp_rvars.txt']), 'w')
    ...
    for sp in temp_rvar_dict[rep]:
        for rvar in temp_rvar_dict[rep][sp]:
            if sp !='farm':
                ...
                sp_name = sp
            temp_rvar_outfile.write('\t'.join([str(args.coreid), str(rep), sp_name, rvar, str(temp_rvar_dict[rep][sp][rvar])]))
